...
| `WordStudyScreen` | Full offline Word Study route with an immediate wrapping ayah selector, route-driven selection, and a Morphology-first tab structure using the same sliding segmented-control pattern as Home. Morphology pairs contextual meaning with segmented Arabic, presents lemma/root facts, shows six form-specific principal parts for supported verbs, and then explains the encountered segments and features. | `app/study/word/[surah]/[ayah]/[position].tsx` |
...
| `VerbReferenceSection` | Phase 11B responsive six-card perfect, imperfect, imperative, active participle, passive participle, and verbal-noun reference for the exact encountered verb form, with cancellable lookup and explicit missing/ambiguous states. | `components/word-study/full-study/VerbReferenceSection.tsx` |
