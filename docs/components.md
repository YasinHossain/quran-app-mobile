...
| `WordStudyScreen` | Full offline Word Study route with an RTL ayah ribbon, route-driven/adjacent selection, Overview, Morphology, Grammar, and Occurrences tabs. Grammar prioritizes the source passage matching the selected Arabic word or morpheme and keeps the complete ayah analysis behind a disclosure. | `app/study/word/[surah]/[ayah]/[position].tsx` |
