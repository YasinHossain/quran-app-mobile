...
| `WordStudyScreen` | Full offline Word Study route with an immediate wrapping ayah selector, route-driven selection, and a Morphology-first tab structure. Morphology pairs contextual meaning on the left with colored Arabic on the right, omits inline language-status badges while preserving source/fallback context in the accessible meaning announcement, wraps readable segment tiles two per row, centers prominent Lemma/Root values in subtle tiles, renders unavailable lexical values as a quiet dash while announcing their full reason, and presents centered unique morphology facts inside clean borderless segment surfaces before progressive-disclosure help. | `app/study/word/[surah]/[ayah]/[position].tsx` |
