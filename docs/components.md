...
| `NativeSurahReader` | Android Kotlin-backed Surah translation reader surface for plain, word-by-word, and Tajweed rows. React Native still owns data loading, settings, verse actions, scrubber, and audio controls while Kotlin owns row rendering and native scrolling. | `components/surah/native/NativeSurahReader.tsx` |
