...
| `WordStudyScreen` | Full offline Word Study route with a wrapping full-ayah selector, route-driven selection, and a horizontally scrollable Overview/Morphology/Grammar/Occurrences/Dictionary tab list. | `app/study/word/[surah]/[ayah]/[position].tsx` |
...
| `wordStudyScreenModel` | Pure presentation model for applicable-only morphology rows, beginner explanations, root/lemma handling, source grouping, and attributed share copy. | `components/word-study/full-study/wordStudyScreenModel.ts` |
