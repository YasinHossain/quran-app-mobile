...
| `WordStudyScreen` | Full offline Word Study route with an immediate wrapping ayah selector, route-driven selection, and a Morphology-first tab structure. Morphology resolves contextual meaning from the selected installed word language with an explicit bundled-English fallback, then shows compact lemma/root facts, unique segment features, and progressive-disclosure terminology help. | `app/study/word/[surah]/[ayah]/[position].tsx` |
