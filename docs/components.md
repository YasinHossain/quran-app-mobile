...
| `WordStudyScreen` | Full offline Word Study route with an immediate wrapping ayah selector, route-driven selection, and a Morphology-first tab structure. Morphology pairs contextual meaning with segmented Arabic, presents lemma/root facts, shows six form-specific principal parts for supported verbs, and then explains the encountered segments and features. | `app/study/word/[surah]/[ayah]/[position].tsx` |
