...
- **Phase 11B — Verb reference pack:** implemented as a separate root-and-form-keyed SQLite pack supplying perfect, imperfect, imperative, active participle, passive participle, and verbal noun. The Morphology view shows only the exact encountered derived form, never every derived form in the root family. Form-I conflicts return an explicit unavailable state instead of being guessed, and Lane/Hans prose is not parsed into canonical paradigms. Public distribution remains gated on source permission and qualified review.
